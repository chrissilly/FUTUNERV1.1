...
| 4.3 | Data logging (gzip + cloud sync) | 🟢 PASS | 🟡 PARTIAL | **Entire dongle-side chain proven on RS7 KOEO 2026-05-26**: capture → gzip → /cal/wot file → HTTPS POST to cloud. Fixed P-63 (reconfigure-on-short-response), P-64 (gzip via deflate_stored), P-65 (`/storage/wot`→`/cal/wot`), P-66 (uploader tick was never called in production). Upload now POSTs and gets a definitive **HTTP 404** — the cloud has no `/api/v1/telemetry/log` route (P-67, cloud-side + deploy). Flips 🟢 once that endpoint is built + deployed; the retained log auto-uploads on the next tick. |
